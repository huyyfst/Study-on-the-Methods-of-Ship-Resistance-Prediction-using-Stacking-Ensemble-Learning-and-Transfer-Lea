...
def input_split_scaler_output_data(file_name, sheet_name):
    Train_set = pd.read_excel('1st_train2.xls', 'Sheet1')
    print('Train_set =\n',Train_set)
    tra_set = Train_set.loc[:, ['dis*1000/L3', 'B/T', 'Cp', 'Cm', 'Cw', 'Lcb/Lbp', 'Lca/Lbp', 'Fr']]
    print('tra_set=\n',tra_set)
    SS = StandardScaler().fit(tra_set)

    Data_set = pd.read_excel(file_name, sheet_name)
    dat_set = Data_set.loc[:, ['dis*1000/L3', 'B/T', 'Cp', 'Cm', 'Cw', 'Lcb/Lbp', 'Lca/Lbp', 'Fr', 'Cr*1000']]
...
test_set_feature,test_set_label = input_split_scaler_output_data('1st_test_KCS.xls', 'Sheet1')
X_test = test_set_feature
Y_test = np.array(test_set_label)#.reshape(14,1)
'''-----------------------------------------*load trained model*---------------------------------'''
# f = open('models\\1st_vc2_KNN.pickle', 'rb') #load model
# f = open('models\\1st_vc2_LR.pickle', 'rb')
# f = open('models\\1st_vc2_SVR.pickle', 'rb')
f = open('models\\1st_vc2_RF.pickle', 'rb')
regressor_model = pickle.load(f)
f.close()
'''-----------------------------------------*Export forecast results of test samples*---------------------------------'''
# pd.DataFrame(regressor_model.predict(X_test)).to_excel('1st_prediction_results\\vc2_KNN.xls')
# pd.DataFrame(regressor_model.predict(X_test)).to_excel('1st_prediction_results\\vc2_LR.xls')
# pd.DataFrame(regressor_model.predict(X_test)).to_excel('1st_prediction_results\\vc2_SVR.xls')
pd.DataFrame(regressor_model.predict(X_test)).to_excel('1st_prediction_results\\vc2_RF.xls')
