...
def input_split_scaler_output_data(file_name, sheet_name):
    Train_set = pd.read_excel('1st_train3.xls', 'Sheet1')
    print('Train_set =\n',Train_set)
...
'''-----------------------------------------*load trained model*---------------------------------'''
# f = open('models\\1st_vc3_KNN.pickle', 'rb') #load model
# f = open('models\\1st_vc3_LR.pickle', 'rb')
# f = open('models\\1st_vc3_SVR.pickle', 'rb')
f = open('models\\1st_vc3_LR.pickle', 'rb')

regressor_model = pickle.load(f)
f.close()
'''-----------------------------------------*Export forecast results of test samples*---------------------------------'''
# pd.DataFrame(regressor_model.predict(X_test)).to_excel('1st_prediction_results\\vc3_KNN.xls')
# pd.DataFrame(regressor_model.predict(X_test)).to_excel('1st_prediction_results\\vc3_LR.xls')
# pd.DataFrame(regressor_model.predict(X_test)).to_excel('1st_prediction_results\\vc3_SVR.xls')
pd.DataFrame(regressor_model.predict(X_test)).to_excel('1st_prediction_results\\vc3_RF.xls')
