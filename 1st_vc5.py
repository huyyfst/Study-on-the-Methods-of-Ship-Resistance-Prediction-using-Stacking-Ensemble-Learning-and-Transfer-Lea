...
def input_split_scaler_output_data(file_name, sheet_name):
    Train_set = pd.read_excel('1st_train5.xls', 'Sheet1')
    print('Train_set =\n',Train_set)
...
'''-----------------------------------------*load trained model*---------------------------------'''
# f = open('models\\1st_vc5_KNN.pickle', 'rb') #load model
# f = open('models\\1st_vc5_LR.pickle', 'rb')
# f = open('models\\vc5_SVR.pickle', 'rb')
f = open('models\\1st_vc5_RF.pickle', 'rb')

regressor_model = pickle.load(f)
f.close()
'''-----------------------------------------*Export forecast results of test samples*---------------------------------'''
# pd.DataFrame(regressor_model.predict(X_test)).to_excel('1st_prediction_results\\vc5_KNN.xls')
# pd.DataFrame(regressor_model.predict(X_test)).to_excel('1st_prediction_results\\vc5_LR.xls')
# pd.DataFrame(regressor_model.predict(X_test)).to_excel('1st_prediction_results\\vc5_SVR.xls')
pd.DataFrame(regressor_model.predict(X_test)).to_excel('1st_prediction_results\\vc5_RF.xls')
